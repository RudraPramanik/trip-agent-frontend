...
The Generate control MUST remain enabled at every readiness tier shown in this phase. Activating it MUST NOT call `POST /api/v1/planner/generate`, MUST NOT open an SSE client, and MUST preserve `?destination=`. Helper text MAY say compose is next.

#### Scenario: Generate click does not generate

- **WHEN** the visitor activates Generate on a selected destination (including `sparse`)
- **THEN** the network has no `POST /api/v1/planner/generate` and the `destination` query param remains
